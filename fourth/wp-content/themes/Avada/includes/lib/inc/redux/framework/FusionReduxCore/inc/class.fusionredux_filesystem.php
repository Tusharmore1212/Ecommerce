...
if ( ! class_exists( 'FusionRedux_Filesystem' ) ) {
	class FusionRedux_Filesystem {
		...
		public $parent = null; // Initialize as null

		public function __construct() {
			$this->parent = new stdClass(); // Initialize $parent as an empty object
			$this->parent->admin_notices = array(); // Initialize admin_notices as an empty array
		...
		public static function get_instance( $parent = null ) {
			...
			if ( $parent !== null ) {
				...
				self::$instance->parent = new stdClass(); // Initialize $parent as an empty object if not provided
			}
		...
		function filesystem_init( $form_url, $method = '', $context = false, $fields = null ) {
			...
			if ( false === ( $this->creds = request_filesystem_credentials( $form_url, $method, false, $context ) ) ) {
				$this->creds = array();
				$this->parent->ftp_form = ob_get_clean(); // Clean buffer and get the contents
		...
		public function do_action( $action, $file = '', $params = '' ) {
			...
			if ( $action == 'mkdir' ) {
				...
				$res = $wp_filesystem->move( $file, $destination, $overwrite );
			} elseif ( $action == 'delete' ) {
				$res = $wp_filesystem->delete( $file, $recursive );
			} elseif ( $action == 'dirlist' ) {
				$include_hidden = isset( $params['include_hidden'] ) ? $params['include_hidden'] : true;
